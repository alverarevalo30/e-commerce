import React from "react";
import { assets } from "../assets/assets";
import { RiExchangeFundsFill } from "react-icons/ri";
import { PiSealCheck, PiHeadset } from "react-icons/pi";
import { motion } from "motion/react";

const OurPolicy = () => {
  return (
    <motion.div
      className="flex flex-col sm:flex-row justify-around gap-12 sm:gap-2 text-center py-20 text-xs sm:text-sm md:text-base text-gray-700"
      initial={{ opacity: 0, y: 50 }}
      whileInView={{ opacity: 1, y: 0 }}
      transition={{
        duration: 0.6,
        ease: "easeOut",
      }}
      viewport={{ once: true, amount: 0.3 }}
    >
      <div>
        <RiExchangeFundsFill className="w-12 h-12 m-auto mb-5" />
        <p className="font-semibold">Easy Exchange Policy</p>
        <p className="text-gray-400">We offer hassle free exhange policy</p>
      </div>
      <div>
        <PiSealCheck className="w-12 h-12 m-auto mb-5" />
        <p className="font-semibold">7 Days Return Policy</p>
        <p className="text-gray-400">We provide 7 days free return policy</p>
      </div>
      <div>
        <PiHeadset className="w-12 h-12 m-auto mb-5" />
        <p className="font-semibold">Best Customer Support</p>
        <p className="text-gray-400">We provide 24/7 customer support</p>
      </div>
    </motion.div>
  );
};

export default OurPolicy;
